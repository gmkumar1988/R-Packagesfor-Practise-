...
pe = data.frame(ABTdata,variant = c("A","B"), conversion_rate = round(c(conversion_rateA, conversion_rateB),4), lower_confidence = round(c(ci_lowera,ci_uppera),4))
pe
...
#Let's plot the distributions
b <- ggplot(ABTdata, aes(x = conf_interval))
